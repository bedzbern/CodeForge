...
const getStudentIp_1 = require("./getStudentIp");
class SidebarProvider {
    ...
    async _handleAsk(question, codeSnapshot) {
        if (!this._view) {
            return;
        }
        const config = vscode.workspace.getConfiguration('codeforge');
        const serverUrl = config.get('serverUrl', 'http://192.168.1.1:8000');
        const editor = vscode.window.activeTextEditor;
        const fileName = editor?.document.fileName ?? 'untitled';
        const lineNumber = editor?.selection.active.line ?? 0;
        const studentIp = (0, getStudentIp_1.getStudentIp)();
        this._view.webview.postMessage({ type: 'loading' });
        try {
            const result = await (0, apiClient_1.askQuestion)(serverUrl, studentIp, question, codeSnapshot, fileName, lineNumber);
...
SidebarProvider.viewType = 'codeforge.askPanel';
